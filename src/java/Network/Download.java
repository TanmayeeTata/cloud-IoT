...
public class Download extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("select * from upload where id='" + fileid + "'");

            if (rs.next()) {
                fname = rs.getString("filename");
//           fkey=rt.getString("secret_key");
                is = (InputStream) rs.getAsciiStream("dfile");
            } else {
            ...
            st1.executeUpdate("insert into download(title,rname,time)values('" + fname + "','" + uname + "','" + time + "')");
